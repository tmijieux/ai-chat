...
`sync-locale-directory` — loops over every locale file in a directory against one authoritative source file, and for each one: finds entries that are either missing entirely or still untranslated (left equal to the key or to the source value), translates just that delta by reusing the [[Locale Translation Workflow]], verifies the translated values didn't break a placeholder or leak a key's context annotation and retranslates any offender once, repairs any keys a translation pass corrupted (e.g. smart punctuation getting normalized), and merges the result back into the real file. Each target file's language is derived from its filename's ISO 639-1 code; a file whose filename isn't a recognized code is reported and skipped rather than guessed.

A locale key may carry a placeholder for app-side substitution, written as a tag such as `{nb}` between curly braces (also `%s`/`%d`/`%1$s`-style and HTML tags), and/or a `[ctx: ...]` prefix giving the model disambiguating context for an otherwise ambiguous or single-word key (e.g. distinguishing the short form of a month name from the number seven). A translated value must keep every placeholder tag the key has (it may drop ones a given language doesn't need, but never add, rename, or invent one) and must never repeat the `[ctx: ...]` annotation itself — that prefix exists only to help translation, not to appear in the app's UI. Values violating either rule are caught and retranslated once before the sync finishes; any that still violate after the retry are reported so a human can look.

Runs unattended across the whole directory — no per-file confirmation, so a directory of many files doesn't produce a wall of prompts. Every file that gets merged is backed up first. A `dry_run` request translates the delta to a scratch file without merging it into the real locale file. The final summary reports counts only (files checked, gaps found/translated, files skipped) — never the translated content itself, dry-run or not, since a directory of any size would overflow the model's context. Invoked via the [[Slash Command Palette]] with a free-text request naming the locale directory and the source filename.
...
**Structure:** the whole workflow plan is drawn as soon as the run starts, before anything executes, then decorated with live status. Each stage row shows: status (pending, running, done, failed, stopped, skipped), stage type, a one-line summary of what the stage produced, its measured token count, and its duration. A stage that runs more than once — because a branch jumped back to it, or because it is inside a loop — shows which invocation is being displayed. **Stopped** is its own status, distinct from failed — it means the user cancelled the run (the same stop button used for a regular agent turn), not that anything went wrong; whatever stage or loop item was in flight at the moment of cancellation is shown as stopped rather than left spinning forever.
...
**Live-following is not persisted:** a page refresh loses the live, streaming view of a run that's still in progress — the backend keeps running, but the frontend can't reconnect mid-stream. Once a run has finished, failed, or been stopped, though, its message's "Reopen workflow run" action brings the same view back read-only, built from disk (see Reopening a past run, above).

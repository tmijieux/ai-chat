...
export class ChatService {
  ...
  async runRagCommand(kind: RagCommandName, arg: string, commandLabel?: string): Promise<void> {
    ...
    const displayInput =
      commandLabel === undefined ? arg : arg !== '' ? `/${commandLabel} ${arg}` : `/${commandLabel}`
    const userMsg: DisplayMessage = { kind: 'user', id: crypto.randomUUID(), content: displayInput }
    this._messages.update((msgs) => [...msgs, userMsg])
    this._promptTokens.set(0)
    ...
    try {
      ...
      if (workspace === null) {
        await this._postRagResultMessage('No workspace configured for this conversation — set one in the settings drawer before using RAG commands.')
        return
      }
  ...
  private async _postRagResultMessage(content: string): Promise<void> {
    ...
    await firstValueFrom(
      this.api.post_message(this._conversation.id, { id: assistantId, role: 'assistant', content }),
    )
    this._messages.update((msgs) => [...msgs, { kind: 'assistant', id: assistantId, content, streaming: false }])
  }

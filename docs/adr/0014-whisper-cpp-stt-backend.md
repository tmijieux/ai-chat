...
### Correction after initial deployment: audio conversion moved from server-side `--convert` to a Python-side ffmpeg pipe

The initial version used `whisper-server`'s built-in `--convert` flag, which writes the upload to a temp file and shells out to ffmpeg via `std::system()`. In real use (real French dictation, not synthetic test clips) this intermittently failed with `{"error":"FFmpeg conversion failed."}`. The server's own log (which captures `std::system()`'s inherited stdout/stderr) showed the actual ffmpeg error: `[ogg @ ...] Codec not found` / `failed to create or replace stream` — ffmpeg correctly identified the Ogg container but sometimes failed to detect the Opus codec inside it. Confirmed this is ffmpeg's own format-probing flakiness, not a concurrency bug on whisper-server's side (`/inference` is fully serialized behind a single mutex, held from the first line of the handler through the conversion step). Also noticed in passing, though not the root cause: `generate_temp_filename`'s Windows path separator was being mis-rendered (an upstream `wchar_t`-into-`ostringstream` bug specific to `std::filesystem::path::preferred_separator` on Windows), landing temp files directly in `%LOCALAPPDATA%` instead of `%TEMP%` — cosmetically wrong but not what broke transcription, since those consistently-malformed paths still worked in the large majority of requests.

Fixed by moving audio conversion into Python, reusing `whisper_pipeline.py`'s already-proven approach: an in-memory ffmpeg pipe (stdin/stdout, no temp files, no `cmd.exe` shell layer) that has never shown this failure mode for the OpenVINO backend. `whisper-server` is now started without `--convert` and receives an already-converted WAV directly — its own reader (`miniaudio`) accepts WAV natively, so no server-side ffmpeg involvement remains at all. Validated with 10 repeated calls on a synthetic short (0.7s) Ogg/Opus clip — matching the buffer size of an early streaming-partial call — with zero failures, versus the frequent failures real dictation was hitting before.
